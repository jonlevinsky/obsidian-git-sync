...
project_tags: fu
status: archived
description: Dějiny filmu — 12+ lekcí od počátků po novou vlnu
...
const folder = dv.pages('"Škola/PANSKÁ/Dějiny Filmu"').where(p => p.file.name !== 'Dějiny Filmu.md').sort(p => p.file.name, 'asc');
...
left.createEl('h1', { text: 'DĚJINY FILMU' });
